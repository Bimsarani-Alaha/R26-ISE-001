...
if result.keypoints is not None:
    ...
    #show result in outside
        ...
        # Draw lines
        cv2.line(annotated_img, ls, rs, (0, 255, 0), 2)   # shoulders
        cv2.line(annotated_img, lh, rh, (255, 0, 0), 2)   # hips
        cv2.line(annotated_img, eye_pt, foot_pt, (0, 0, 255), 2)  # height

        # Draw text
        cv2.putText(annotated_img, f"S:{shoulder_width:.1f}",
                    (ls[0], ls[1] - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

        cv2.putText(annotated_img, f"H:{hip_width:.1f}",
                    (lh[0], lh[1] - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)

        cv2.putText(annotated_img, f"HT:{height:.1f}",
